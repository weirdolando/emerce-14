...
module.exports = {
    ...
    getProduct: (req, res) => {
        ...
        var orderBy = ''
        if(req.query.sort != null || req.query.top != null || req.query.price !=null){
            orderBy = 'order by '
        }
        ...
        db.query(`select * from products ${where}${name}${and}${category}${orderBy}${sort}${price}limit 9 offset ${(page-1)*9}`, (err, result) => {
    ...
    getTotalProduct: (req, res) => {
        ...
        db.query(`select count(*) as total_products from products ${where}${name}${and}${category}`, (err, result) => {
            if(err) res.status(400).send(err)
            res.send({
                status: 200,
    ...
    addProduct: (req, res) => {
        const { store_id, category_id, product_name, price, image, description, stock } = req.body
        db.query(
            `insert into products(store_id, category_id, product_name, price, image, description, stock, is_active, sold) values
            (${db.escape(store_id)}, ${db.escape(category_id)}, ${db.escape(product_name)}, ${db.escape(price)}, ${db.escape(image)}, ${db.escape(description)},  ${db.escape(stock)}, 1, 0);`, (err) => {
            if(err) res.status(400).send(err)
            res.send({
                status: 200,
